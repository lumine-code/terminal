const { detectLinks, fallbackPathMatchers } = require("../lib/link-detection/path-parsing");
...
describe("fallbackPathMatchers", () => {
  ...
  it("matches a clang-style path:line:column suffix without swallowing the line", () => {
    let groups = firstMatch("/foo/bar baz.c:339:12: error: expected ;");
    expect(groups.path).toBe("/foo/bar baz.c");
    expect(groups.line).toBe("339");
    expect(groups.col).toBe("12");
  });

  it("matches a spaced path:line:column suffix at the end of a line", () => {
    let groups = firstMatch("/foo/bar baz.c:339:12");
    expect(groups.path).toBe("/foo/bar baz.c");
    expect(groups.line).toBe("339");
    expect(groups.col).toBe("12");
  });
