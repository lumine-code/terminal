...
describe("ShellIntegrationAddon", () => {
  ...
  describe("Cwd", () => {
    ...
    it("unescapes semicolons, backslashes, and control characters", async () => {
      let spy = jasmine.createSpy("cwd-spy");
      addon.onDidChangeCwd(spy);
      // A path like `/tmp/a;b\c` would be escaped by `__lumine_escape_value`
      // as `/tmp/a\x3bb\\c`.
  ...
  it("reports the start of a prompt", async () => {
    let spy = jasmine.createSpy("prompt-spy");
    addon.onDidStartPrompt(spy);
    await write(terminal, "\x1b]633;A\x07");
  ...
  describe("command detection", () => {
    ...
    it("does not attribute a command line when the nonce is missing", async () => {
      ...
      expect(spy.calls.argsFor(0)[0].commandLine).toBeUndefined();
    });
    ...
    it("reports a finished command with its exit code", async () => {
      ...
      let reported = spy.calls.argsFor(0)[0];
    ...
    it("reports exitCode undefined when D carries no exit code", async () => {
      ...
      expect(spy.calls.argsFor(0)[0].exitCode).toBeUndefined();
    });
    ...
    it("drops command and cwd state when a new PTY session supplies a nonce", async () => {
      let finished = jasmine.createSpy("finish-spy");
      let executed = jasmine.createSpy("execute-spy");
      addon.onDidFinishCommand(finished);
      addon.onDidExecuteCommand(executed);
      await write(terminal, "\x1b]633;P;Cwd=/old/session\x07");
      await write(terminal, `\x1b]633;E;old command;${NONCE}\x07`);
      await write(terminal, "\x1b]633;C\x07");

      addon.setNonce("new-nonce");
      await write(terminal, "\x1b]633;D;0\x07");
      await write(terminal, "\x1b]633;C\x07");

      expect(finished).not.toHaveBeenCalled();
      expect(executed.calls.mostRecent().args[0]).toEqual({
