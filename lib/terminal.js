...
// `menus/terminal.json`, which decides where the items appear; this one decides
  ...
  static activate(_state) {
    ...
    const resolveContextMenuOptions = (target) => {
      ...
      if (target.closest(".tree-view")) {
        ...
        let container = lumine.workspace
          .getPaneContainers()
          .find((paneContainer) => paneContainer.getElement().contains(target));
        if (container) result.location = container.getLocation();
      }
