...
const VALID_LOCATIONS = ["center", "left", "right", "bottom"];
...
class Terminal {
  ...
  static activate(_state) {
    ...
    function resolveOptionsForContextMenuSplit(target) {
      ...
      if (target?.closest(".tree-view")) {
        result.target = lumine.workspace.getActiveTextEditor()?.getElement();
        let li = target.closest('li[is="tree-view-directory"]');
        if (li) {
          result.cwd = li.querySelector("[data-path]")?.dataset?.path;
        }
      } else {
        let paneContainerElement = target.closest("lumine-pane-container, lumine-panel-container");
        if (paneContainerElement) {
          if (paneContainerElement.matches("lumine-panel-container")) {
            let location = paneContainerElement.className;
            if (isValidLocation(location)) {
              result.location = location;
            }
          } else {
            // Must be an `lumine-pane-container`, meaning it's the workspace
            // center.
    ...
    this.subscriptions.add(
      ...
      lumine.commands.add("terminal-view", {
        ...
        "terminal:previous-command": (event) => {
          this.inferTerminalElement(event)?.scrollToPreviousCommand();
        },
        "terminal:next-command": (event) => {
      ...
      lumine.commands.add("lumine-text-editor:not([mini]), .tree-view, .tab-bar", {
  ...
  static async open(uri, options = {}) {
    ...
    Logger.debug("Opening with options:", options, url.href);

    return await lumine.workspace.open(url.href, options);
  }
  ...
  static getPath(target) {
    ...
    if (treeView) {
      ...
      return (
        treeView.querySelector(".entry.selected:not(.tree-view-special-root)")?.getPath?.() ?? null
      );
    }
