...
class Terminal {
  ...
  static activate(_state) {
    ...
    const resolveContextMenuOptions = (target) => {
      let result = {};
      if (!lumine.dom.isElement(target)) return result;
  ...
  static inferTerminalElement(event) {
    if (!lumine.dom.isElement(event?.target)) return null;
  ...
  static async open(uri, options = {}) {
    ...
    if (lumine.dom.isElement(opts.target) && !url.searchParams.has("cwd")) {
