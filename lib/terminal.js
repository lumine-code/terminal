...
class Terminal {
    static activate(_state) {
        ...
        function resolveOptionsForContextMenuSplit(target) {
            let result = {};
            ...
            if (target?.closest('.tree-view')) {
                ...
                if (li) {
                    result.cwd = li.querySelector('[data-path]')?.dataset?.path;
                }
            }
            else {
                let paneContainerElement = target.closest('atom-pane-container, atom-panel-container');
        ...
        atom.views.addViewProvider(model.TerminalModel, (model) => {
            let element$1 = new element.TerminalElement();
            element$1.initialize(model);
            return element$1;
        }), 
        // Add an opener for the terminal emulator.
        atom.workspace.addOpener((uri) => {
            if (!uri.startsWith(utils.BASE_URI))
                return undefined;
            let item = new model.TerminalModel({
                uri,
        ...
        atom.workspace.observePanes((pane) => {
            this.subscriptions.add(pane.observeItems((item) => {
                if (model.TerminalModel.is(item)) {
                    item.moveToPane(pane);
                }
                utils.recalculateActive(this.terminals);
            }));
            utils.recalculateActive(this.terminals);
        }), 
        ...
        let debouncedUpdateTheme = utils.debounce(() => this.updateTheme());
        this.subscriptions.add(
    ...
    static async canRunCommands(commands) {
        ...
        if ((config.Config.get('advanced.allowedCommands') ?? []).includes(serializedCommands)) {
            return true;
        }
            ...
            let notification = atom.notifications.addInfo('Terminal: Approve commands', {
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        onDidClick() {
                            disposable?.dispose();
                            let allowedCommands = config.Config.get('advanced.allowedCommands') ?? [];
                            config.Config.set('advanced.allowedCommands', [...allowedCommands, serializedCommands]);
                            notification.dismiss();
                            resolve(true);
                        }
                    }
    ...
    static async runCommands(commands) {
        let terminal;
        if (config.Config.get('behavior.runInActive')) {
    ...
    static getPath(target) {
        ...
        if (treeView) {
            let selected = treeView.querySelector('.selected > .list-item > .name, .selected > .name');
        ...
        let textEditor = target.closest('atom-text-editor');
    ...
    static addDefaultPosition(options = {}) {
        let position = config.Config.get('behavior.defaultContainer');
        switch (position) {
            case 'Center': {
    ...
    static deserializeTerminalModel(serializedModel) {
        ...
        // @ts-ignore Undocumented.
        pack.activateNow();
        if (!config.Config.get('behavior.relaunchTerminalsOnStartup')) {
            return;
        }
        return new model.TerminalModel({
    ...
    static generateUri(params = {}) {
        return utils.generateUri(params);
    }
    // SERVICES
    // ========
    ...
    static providePlatformioIDETerminalService() {
        return {
            ...
            updateProcessEnv: (_vars) => {
                // No-op.
            }
        };
    }
}
Terminal.config = config.getConfigSchema();
