...
const { getMcpBridge, setAutoShellPromise, setMcpBridge } = require("./runtime-state");

let configModule;
let utilsModule;
const getConfigModule = () => (configModule ||= require("./config"));
const getUtilsModule = () => (utilsModule ||= require("./utils"));
const BASE_URI = "terminal://";

// The entrypoint is loaded while the package is activated, before any
// terminal has been opened. Keep the xterm renderer, its addons and the PTY
// worker behind these accessors so activation only installs the public
// commands/openers. The first view (or operation that needs the model) pays
// for the corresponding runtime once, and concurrent callers share the same
// module cache.
let terminalElementModule;
let terminalModelModule;
let ptyModule;
let autoShellPromise;

function getTerminalElement() {
  if (!terminalElementModule) {
    // Etch holds its scheduler per copy of the library, and this package
    // resolves its own copy — so the assignment the editor makes on core's
    // copy never reaches it. Configure it immediately before the first view
    // module is loaded, rather than loading etch during package activation.
    require("@lumine-code/etch").setScheduler(lumine.views);
    terminalElementModule = require("./element");
    // `element.js` imports the PTY implementation as part of the same lazy
    // boundary. Keep its module handle so deactivation can release a host
    // that was actually started, without requiring PTY for an unused package.
    ptyModule ??= require("./pty");
  }
  return terminalElementModule.TerminalElement;
}

function getTerminalModel() {
  terminalModelModule ??= require("./model");
  return terminalModelModule.TerminalModel;
}

function getIsSafeSignal() {
  terminalModelModule ??= require("./model");
  return terminalModelModule.isSafeSignal;
}

function isTerminalModel(item) {
  return getTerminalModel().is(item);
}

function ensureAutoShell() {
  if (!autoShellPromise) {
    autoShellPromise = getConfigModule()
      .possiblySetAutoShell()
      .catch((error) => {
        // Shell auto-detection is a convenience. A failed lookup must not keep
        // commands from being registered or make opening a terminal reject.
        Logger.warn("Unable to detect the default shell:", error);
      });
    setAutoShellPromise(autoShellPromise);
  }
...
const CONTEXT_MENU_SCOPES = [
  ...
  ".tree-view .full-menu .entry:not(.tree-view-special-root)",
  "lumine-pane[data-active-item-path] .tab.active",
];

class Terminal {
  static provideBackgroundTips() {
    return {
      packageName: "terminal",
      tips: [
        "You can open a terminal beside your code with {{ 'terminal:open' | keystroke }}",
        "You can run the selected text in a terminal with {{ 'terminal:run-selected-text' | keystroke }}",
      ],
    };
  }
  ...
  static activate(_state) {
    ...
    // Terminal models are lightweight workspace items and are needed to
    // register the synchronous opener/deserializer. The DOM renderer is
    // deliberately required only inside the view provider below.
    const TerminalModel = getTerminalModel();

    // The workspace observers below report all existing panes, items and dock
    // visibilities as soon as they are registered. Until a terminal exists,
    // none of those notifications can affect terminal ordering. Besides doing
    // needless work, recalculating an empty set pulls the configuration and
    // shell-detection helpers into the synchronous activation path.
    const recalculateActive = () => {
      if (this.terminals.size === 0) return;
      getUtilsModule().recalculateActive(this.terminals);
    };

    Logger.initialize();

    // Attempt to detect the correct shell on Windows, but do not make package
    // activation wait for a filesystem lookup. Calls that create a terminal
    // await the shared promise below before they open one.
    queueMicrotask(() => {
      if (this.activated) void ensureAutoShell();
    });
    ...
    this.subscriptions.add(
      ...
      lumine.views.addViewProvider(TerminalModel, (model) => {
        let element = getTerminalElement().create();
      ...
      lumine.commands.add("terminal-view", {
        "terminal:close": {
          description: "End this terminal's session and close its tab.",
          didDispatch: (event) => this.close(event),
        },
        "core:copy": (event) => {
        ...
        "terminal:unfocus": {
          description: "Return focus to what was active before the terminal.",
          didDispatch: (event) => {
            return this.unfocus(event);
          },
        },
        "terminal:clear": {
      ...
      //
      // These commands name their destination, so they need nothing from the
      // click but the directory. The default-position command is the global
      // `terminal:open` above: sharing it with the keymap lets the context menu
      // show its keystroke.
      ...CONTEXT_MENU_SCOPES.map((selector) =>
        lumine.commands.add(selector, {
          ...
          "terminal:open-split-down-context-menu": {
            didDispatch: ({ target }) => {
              let { cwd } = resolveContextMenuOptions(target);
              this.open(this.generateUri(cwd ? { cwd } : {}), {
                split: "down",
                location: "center",
              });
            },
    ...
    let updateTheme = getUtilsModule().coalesce(() => this.updateTheme());
    ...
    const onFocusIn = (event) => {
      const target = this.describeFocus(event.target);
      if (target) this.previousFocus = target;
    };
    document.addEventListener("focusin", onFocusIn);
    this.subscriptions.add(
      new Disposable(() => document.removeEventListener("focusin", onFocusIn)),
    );
  }

  static describeFocus(target) {
    if (!(target instanceof HTMLElement)) return null;
    if (target.closest("terminal-view")) return null;

    const pane = target.closest("lumine-pane")?.getModel?.();
    const item = pane?.getActiveItem?.();
    if (item) return isTerminalModel(item) ? null : { type: "item", item };
  ...
  static async open(uri, options = {}) {
    // Keep opener/command dispatch synchronous. The terminal element waits for
    // this shared lookup before spawning its shell, so opening a pane never
    // races the Windows shell preference while the workspace can update now.
    void ensureAutoShell();
    let url = new URL(uri);
    ...
    if (opts.target instanceof HTMLElement && !url.searchParams.has("cwd")) {
  ...
  static copy(event) {
    ...
    return lumine.clipboard.write(text ?? "");
  }

  static async paste(event) {
    let model = this.inferTerminalModel(event);
    if (!model) return;

    let textToPaste = await lumine.clipboard.read();
    if (textToPaste) {
   ...
   *
   * Returns a Promise resolving to `true` when a provider claimed the paste.
   */
  static async pasteImage(model, { explicit = false } = {}) {
    if (!model) return false;

    let claimed = await lumine.pasteProviders.handlePaste({
    ...
    if (!(await lumine.clipboard.readImage()).isEmpty()) {
  ...
  static async sendSignal(signal) {
    if (!getIsSafeSignal()(signal)) return;
  ...
  static async canRunCommands(commands) {
    ...
    if (
      (getConfigModule().Config.get("advanced.allowedCommands") ?? []).includes(serializedCommands)
    ) {
      return true;
    }
      ...
      let notification = lumine.notifications.addInfo("Terminal: Approve commands", {
        ...
        buttons: [
          ...
          },
          {
            ...
            onDidClick() {
              disposable?.dispose();
              let allowedCommands = getConfigModule().Config.get("advanced.allowedCommands") ?? [];
              getConfigModule().Config.set("advanced.allowedCommands", [
                ...allowedCommands,
                serializedCommands,
              ]);
              notification.dismiss();
  ...
  static async runCommands(commands) {
    let terminal;
    if (getConfigModule().Config.get("behavior.runInActive")) {
  ...
  static addDefaultPosition(options = {}) {
    ...
    // caller's.
    options = { ...options };
    let position = getConfigModule().Config.get("behavior.defaultContainer");
    switch (position) {
      case "Center": {
  ...
  static deactivate() {
    ...
    // will end it.
    // Do not load the PTY worker module merely to deactivate a package that
    // never opened a terminal.
    ptyModule?.PtyHost.releaseShared();
    Logger.destroy();
  ...
  static deserializeTerminalModel(serializedModel) {
    // The package-owned deserializer proxy requires and initializes this module
    // and installs its runtime schema before invoking the synchronous method.
    if (!getConfigModule().Config.get("behavior.relaunchTerminalsOnStartup")) {
      return;
    }
    return new (getTerminalModel())({
  ...
  static unfocus() {
    ...
    if (target?.type === "item") {
      const pane = lumine.workspace.paneForItem(target.item);
      if (pane) {
    ...
    if (target?.type === "element" && target.element.isConnected) {
      ...
      if (document.activeElement === target.element) return true;
    }

    lumine.workspace.getElement()?.focus();
    return false;
  }

  static generateUri(params = {}) {
    return getUtilsModule().generateUri(params);
  }

  // SERVICES
  // ========

  static consumeMcpBridge(mcpBridge) {
    setMcpBridge(mcpBridge);
    return new Disposable(() => {
      if (getMcpBridge() === mcpBridge) {
        setMcpBridge(null);
      }
    });
  }
