...
class Pty {
  ...
  // so sweep them by hand. `/T` takes the whole tree beneath the shell, which
  // is what a long-running program that spawns its own helpers leaves behind;
  // enumerating the shell's children and killing those, which is what this did
  // before, only ever reached one level of it. The root of the tree is this
  // session's own shell, so nothing outside this terminal is in scope.
  //
  ...
  // that has nothing to say — so the sweep had simply stopped happening.
  #killDescendantsOnWindows() {
    ...
    let killProcess;
    try {
      killProcess = this.spawn("taskkill", ["/PID", String(rootPid), "/T", "/F"], {
        windowsHide: true,
        stdio: "ignore",
      });
    } catch (error) {
      Logger.error("Failed to sweep the terminal's descendants:", error);
      return;
    }

    // A tree that has already gone makes `taskkill` exit non-zero, which is the
    // outcome we wanted anyway. Only an unhandled error event matters here.
