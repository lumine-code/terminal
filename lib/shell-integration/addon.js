...
const lumine_1 = require("lumine");
const OSC_SHELL_INTEGRATION = 633;
// Reverses the escaping done by `__lumine_escape_value` in
...
class ShellIntegrationAddon {
  ...
  #currentCommand;
  activate(terminal) {
    this.#disposable = terminal.parser.registerOscHandler(OSC_SHELL_INTEGRATION, (data) =>
  ...
  onDidStartPrompt(callback) {
    return this.#emitter.on("did-start-prompt", callback);
  }
  ...
  #handleSequence(data) {
    ...
    switch (command) {
      case "A":
        this.#emitter.emit("did-start-prompt");
        return true;
      case "P": {
      ...
      // `B` (prompt end) and `F`/`G` (continuation prompt
