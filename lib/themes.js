...
function getTheme(document = globalThis.document) {
  const domWindow = document.defaultView || globalThis.window;
  ...
  switch (theme) {
    ...
    case "Solid Colors":
      ...
      break;
    case "Standard": {
      let root = domWindow.getComputedStyle(document.documentElement);
...
// `ITheme` object.
function getSearchTheme(document = globalThis.document) {
  const domWindow = document.defaultView || globalThis.window;
  let theme = config.Config.get("appearance.theme");
  let {
    matchBorder,
    ...
    foreground,
    background,
  } = config.Config.get("appearance.customThemeColors") ?? {};
  ...
  switch (theme) {
    case "Config":
    // "Config" uses the custom colors as-is, so it shares the "Stylesheet"
    // handling below.
    // falls through
    case "Stylesheet": {
...
const THEME_COLORS = [
  ...
  },
  {
    ...
    short: "cursorText",
    description:
      "The color of a character when the cursor is on it _and_ the cursor is in “block” mode.",
    default: "#000000",
  },
  {
...
// so the reference captured by `./config` during the config↔themes require
// cycle stays valid.
Object.assign(module.exports, { getTheme, getSearchTheme, THEME_COLORS });
