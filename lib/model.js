...
const ALLOWED_LOCATIONS = ["center", "bottom", "left", "right"];
...
class TerminalModel {
  ...
  static recalculateActive(terminals, active) {
    return require("./utils").recalculateActive(terminals, active);
  }
  ...
  constructor(options) {
    ...
    // What the opener asked for, which may well be a file: a terminal opened
    // from an editor's context menu carries that editor's path. `cwd` holds
    // only a directory, and only once initialization has resolved one — so
    // nothing reads a file path off this item in the meantime.
    this.requestedCwd = this.url.searchParams.get("cwd") ?? undefined;
  ...
  async getInitialCwd() {
    // Filesystem probing is only needed when a terminal model is opened. Keep
    // fs-extra out of the package bootstrap so registering the model,
    // deserializer and opener remains cheap.
    const fs = require("fs-extra");
    let cwd;

    // First, collect candidates for the `cwd`.
    if (this.requestedCwd) {
      ...
      // active pane item.
      let previousActiveItem = lumine.workspace.getActivePaneItem();
      // A terminal has no `getPath` to ask, so take its working directory
      // directly. This is what makes a terminal opened from a terminal start
      // where that one is.
      if (TerminalModel.is(previousActiveItem)) {
        if (previousActiveItem.cwd) return previousActiveItem.cwd;
        cwd = undefined;
      } else {
        cwd = previousActiveItem?.getPath?.() ?? previousActiveItem?.selectedPath;
      }
      if (cwd) {
        let [dir] = lumine.project.relativizePath(cwd);
        if (dir) {
  ...
  async initialize() {
    this.setCwd(await this.getInitialCwd());
  }

  setCwd(newCwd) {
    if (newCwd !== undefined && (typeof newCwd !== "string" || !path.isAbsolute(newCwd))) {
      return false;
    }
    ...
    if (newCwd) {
      ...
      this.url.searchParams.delete("cwd");
    }
    return true;
  }
  ...
  // Pane-item methods this model deliberately does NOT implement. Every one of
  // them is feature-detected by its callers, and every one describes a document
  // — which a terminal is not. Implementing them anyway is how a terminal came
  // to claim things about itself that were not true:
  //
  // - `getPath` names the document an item displays. The tab bar shows it on
  //   hover and colours the tab by its git status, so a terminal opened from an
  //   editor's context menu claimed that editor's file as its own. What a
  //   terminal has is a working directory, and that is `cwd`.
  // - `getFileState` and `onDidChangeFileState` report a document's relationship
  //   with its backing file. The tab bar's dot is only one reader:
  //   `tabs:close-saved-tabs` skips any non-unmodified item, so borrowing the dot
  //   to mean “output you haven't seen” made a terminal survive that command
  //   depending on whether output happened to arrive while it was in the
  //   background.
  // - `shouldPromptToSave` and `save` offer to write the item to disk. There is
  ...
  handleNewData() {
    this.pane ??= lumine.workspace.paneForItem(this) ?? undefined;
  ...
  async waitForElement(timeoutMs = 2000) {
    ...
    return await require("./utils").timeout(promise, timeoutMs);
  }
  ...
  moveToPane(pane) {
    ...
    switch (location) {
      case "left":
        this.dock = lumine.workspace.getLeftDock();
        break;
      case "right":
        this.dock = lumine.workspace.getRightDock();
        break;
      case "bottom":
        this.dock = lumine.workspace.getBottomDock();
        break;
      default:
        this.dock = undefined;
    }
  ...
  copy() {
    return new TerminalModel({
      uri: require("./utils").generateUri(this.getParams()),
   ...
   */
  run(command) {
    this.element?.pty?.write(command + require("os").EOL.charAt(0));
  }
  ...
  setActive() {
    require("./utils").recalculateActive(this.terminals, this);
  }
...
module.exports = { isSafeSignal, TerminalModel };
