...
class TerminalModel {
  ...
  async getInitialCwd() {
    ...
    if (cwd) {
      // Ideally, we'd be able to keep `cwd` up to date even when it's changed
      // via `cd` and other commands. VS Code can only do this by
  ...
  async beginWindowSurfaceTransition(context) {
    ...
    return await this.element.beginWindowSurfaceTransition(context);
  }
