...
class TerminalModel {
  ...
  getPath() {
    // The cwd is resolved during initialization; before that, fall back to the
    // first project root.
    return this.cwd ?? atom.project.getPaths()[0];
  }
  ...
  // A terminal deliberately implements neither `isModified` nor
  // `shouldPromptToSave`. There is nothing here to save, and the tab bar's
  // “modified” dot is a save-state indicator that other commands read as one —
  // `tabs:close-saved-tabs` skips a modified item, so borrowing the dot to mean
  // “output you haven't seen” made a terminal survive that command depending on
  // whether output happened to arrive while it was in the background.
  setElement(element) {
