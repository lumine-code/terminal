...
class TerminalModel {
    ...
    static recalculateActive(terminals, active) {
        return utils.recalculateActive(terminals, active);
    }
    constructor(options) {
        ...
        this.pane = undefined;
        this.dock = undefined;
        _TerminalModel_lastTitle.set(this, void 0);
        let uri = options.uri;
        this.url = new url.URL(uri);
    ...
    async getInitialCwd() {
        ...
        try {
            if (cwd) {
                ...
                let stats = await fs__default.default.stat(cwd);
                if (stats.isDirectory())
                    return cwd;
                // Maybe it's the path to a file? Try its parent directory just to be
                // safe.
                cwd = path__default.default.dirname(cwd);
                let dirStats = await fs__default.default.stat(cwd);
    ...
    getPath() {
        return this.cwd ?? utils.getCurrentCwd();
    }
    getURI() {
        return this.uri;
    }
    ...
    getActiveIndicator() {
        return config.Config.get('terminal.activeTerminalIndicator');
    }
    getIconName() {
        return 'terminal';
    }
    ...
    handleNewData() {
        ...
        this.modified = item !== this;
        if (oldIsModified !== this.modified) {
            this.emitter.emit('did-change-modified', this.modified);
        }
        if (this.title !== tslib.__classPrivateFieldGet(this, _TerminalModel_lastTitle, "f")) {
            this.emitter.emit('did-change-title', this.getTitle());
        }
        tslib.__classPrivateFieldSet(this, _TerminalModel_lastTitle, this.title, "f");
    }
    isActive() {
        const activeLogic = config.Config.get('behavior.activeTerminalLogic');
        return this.activeIndex === 0 && (this.isVisible() || activeLogic === 'all');
    }
    isVisible() {
        if (!this.pane)
            return false;
        if (this.pane.getActiveItem() !== this)
            return false;
        if (!this.dock)
            return true;
    ...
    async waitForElement(timeoutMs = 2000) {
        ...
        return await utils.timeout(promise, timeoutMs);
    }
    moveToPane(pane) {
        this.pane = pane;
    ...
    copy() {
        return new TerminalModel({
            uri: utils.generateUri(this.getParams()),
     ...
     */
    run(command) {
        this.element?.pty?.write(command + os__default.default.EOL.charAt(0));
    }
    /** Clear the screen. */
    clear() {
        this.element?.clear();
    ...
    setActive() {
        utils.recalculateActive(this.terminals, this);
    }
...
_TerminalModel_lastTitle = new WeakMap();

exports.TerminalModel = TerminalModel;
exports.isSafeSignal = isSafeSignal;
