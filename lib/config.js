...
function getConfigSchema() {
  ...
  return {
    ...
    xterm: {
      ...
      properties: {
        ...
        localPathDetection: {
          ...
          description:
            "Detect local filesystem paths in terminal output and make them clickable. Relative paths are resolved against the shell's current working directory.",
          type: "boolean",
          default: true,
        },
        ligatures: {
        ...
        images: {
          title: "Inline Images",
          description: `Render [inline images](https://github.com/xtermjs/xterm.js/tree/master/addons/addon-image) written to the terminal using SIXEL or iTerm's inline image protocol. Lets tools like \`imgcat\`, \`matplotlib\` and \`chafa\` draw in place instead of opening a window.`,
          type: "boolean",
          default: true,
        },
        imageStorageLimit: {
          title: "Inline Image Memory Limit",
          description:
            "Megabytes of decoded inline images kept per terminal. Older images are dropped first and leave a placeholder behind. (Only applies when **Inline Images** is enabled.)",
          type: "integer",
          minimum: 0,
          default: 128,
        },
    ...
    appearance: {
      ...
      properties: {
        fontFamily: {
          ...
          type: "string",
          default: lumine.config.get("editor.fontFamily") || "monospace",
        },
        useEditorFontFamily: {
    ...
    behavior: {
      ...
      properties: {
        ...
        requireModifierToOpenUrls: {
          ...
          description: `When enabled, you must hold down ${utils.isMac() ? "`Cmd`" : "`Ctrl`"} while clicking on a URL or local path in order to open it.`,
          type: "boolean",
          default: true,
        },
        localPathBehavior: {
          title: "Local Path Behavior",
          description: "Choose whether detected filesystem paths open in Lumine or externally.",
          type: "string",
          enum: [
            {
              value: "files-in-lumine",
              description: "Open files in Lumine and directories in the system file manager",
            },
            {
              value: "all-external",
              description: "Reveal files and open directories in the system file manager",
            },
          ],
          default: "files-in-lumine",
        },
      },
    },
    advanced: {
      ...
      properties: {
        shareProcess: {
          title: "Share One Process For All Terminals",
          description:
            "Runs every terminal through a single background process instead of giving each one its own.\n\nEach process costs about 100MB and a tenth of a second to start, so sharing makes every terminal after the first open much faster and keeps the memory flat no matter how many you have open. Turn this off to isolate them: a background process that dies then takes down only its own terminal. Applies to terminals opened from now on.",
          type: "boolean",
          default: true,
        },
        enableDebugLogging: {
          ...
          description:
            "Logs more information from the PTY process to the developer console.\n\nYou may want to enable this if you’re reporting a bug and you’re asked to provide more information. Otherwise, leave it disabled; it’s quite verbose!",
          type: "boolean",
          default: false,
        },
        allowedCommands: {
    ...
    shellIntegration: {
      ...
      description:
        "Integrate automatically with bash, zsh, fish and PowerShell to track the working directory, mark commands, decorate prompts by exit status and enable prompt-to-prompt navigation.",
      type: "object",
      properties: {
        enabled: {
          title: "Enable Shell Integration",
          description:
            "Inject shell integration when the configured shell and arguments are supported, while continuing to recognize external OSC 133 and OSC 633 sequences. (Takes effect after a terminal is restarted.)",
          type: "boolean",
          default: true,
        },
      },
