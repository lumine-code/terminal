...
function getConfigSchema() {
    ...
    for (let [i, item] of themes.THEME_COLORS.entries()) {
        let schema = {};
    ...
    return {
        terminal: {
            ...
            properties: {
                ...
                terminalType: {
                    title: 'Terminal Type',
                    description: 'The type of terminal to report; will be assigned to the `TERM` environment variable.',
                    type: 'string',
        ...
        behavior: {
            ...
            properties: {
                ...
                leaveOpenAfterExit: {
                    ...
                    description: 'When enabled, terminal pane items will remain open even after their shells have exited. When disabled, terminal pane items will be removed from the workspace immediately upon shell exit.',
                    type: 'boolean',
                    default: false,
                    order: 5
                },
...
async function setAutoShell() {
    ...
    let command = await which__default.default('pwsh.exe', { nothrow: true });
    command ??= await which__default.default('powershell.exe', { nothrow: true });
    if (!command)
        return;
...
exports.possiblySetAutoShell = possiblySetAutoShell;
