...
const { getSearchTheme } = require("./themes");
...
class FindPalette {
  ...
  get document() {
    return this.element?.ownerDocument || this.hostDocument || globalThis.document;
  }

  getDecorationsOptions() {
    const document = this.document;
    let root = document.defaultView.getComputedStyle(document.documentElement);
    let options = {
      ...
      // These get specified by the stylesheet. There's not currently an analog
      // to this approach if a user chooses to define colors via the config
      // system.
  ...
  constructor(searchAddon, { document = globalThis.document } = {}) {
    ...
    let updateSearchTheme = coalesce(() => (this.searchTheme = getSearchTheme(this.document)));

    // Cache the theme colors that relate to the find palette…
    this.searchTheme = getSearchTheme(this.document);

    // …but invalidate the cache whenever the settings change.
    this.subscriptions.add(
      lumine.config.onDidChange("terminal.appearance", updateSearchTheme),
      lumine.themes.onDidChangeActiveThemes(updateSearchTheme),
      lumine.styles.onDidAddStyleElement(updateSearchTheme),
    );
  }
  ...
  render() {
    ...
    let result = $.div(
      { ref: "element", className: "terminal-find-palette tool-panel" },
