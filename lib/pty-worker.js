...
//
// `@lumine-code/node-pty` is this package's own dependency, so the `require`
// below resolves through the ordinary `node_modules` walk from this file. It is
// a native module, which means installing this package compiles one — the
// addon is Node-API, so the build is ABI-stable across the editor's Electron
// and any Node it was built against.
...
async function processMessage(data) {
  ...
  switch (data.type) {
    ...
    case "resize": {
      ...
      // The renderer validates sizes before sending, but this process is the
      // last line of defense: `node-pty` throws on a non-positive size, a
      // throw here is an uncaught exception, and one worker serves every
      // terminal in the window — a single bad resize must not take them all
      // down. (JSON flattens NaN to null in transit, so a size that was
      // merely unmeasurable arrives actively invalid.) Dropping the resize
      // keeps the current size, which the next valid resize corrects.
      if (!Number.isInteger(cols) || cols <= 0 || !Number.isInteger(rows) || rows <= 0) {
        log(`Ignoring an invalid resize for session ${id}: ${cols}x${rows}`);
        return;
      }
