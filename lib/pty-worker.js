...
// process’s Node.
//
// `node-pty` is deliberately NOT a dependency of this package. Lumine ships it
// and re-exports it from its `exports/` folder, which is on `NODE_PATH`, so the
// `require` below resolves to the editor's copy. That keeps installing this
// package from compiling a native module, which is where such installs usually
// fail. It also means the worker must inherit `NODE_PATH` from the renderer —
// see `Pty.start` in `pty.js`.
