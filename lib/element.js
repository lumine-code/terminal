...
const { CompositeDisposable, Disposable, KeyBinding } = require('atom');
const { isSafeSignal, Signal, TerminalModel } = require('./model');
...
class TerminalElement extends HTMLElement {
  ...
  #keyboardEventMatchesKeybinding (event) {
    ...
    let result = bindings.exactMatchCandidates.some((kb) => this.#shouldPrioritizeBinding(kb, ancestorChain));
