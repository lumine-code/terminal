...
// `@electron/remote` is declared as an explicit dependency so the package works
// when installed on its own. Once it ships bundled with the editor,
// `@electron/remote` is ambiently available and the explicit dependency can be
// dropped to avoid version clashes.
...
// width. The probe is appended inside `atom-workspace` so it inherits the same
...
function measureEditorScrollbarWidth(preferredHost) {
  let host = preferredHost?.isConnected ? preferredHost : atom.workspace.getElement();
...
class TerminalElement extends HTMLElement {
  ...
  // The ruler itself is kept invisible by theming its border to the background
  // color (see `lib/themes.js`); it still hosts the search-result markers.
  ...
  updateTheme() {
    ...
    this.terminal.options.overviewRuler = this.getOverviewRulerOptions();
  }
