...
// first, the redrawn content afterwards. Each read chunk crosses the pty
// worker's IPC boundary as its own message and is written to xterm on its
// own, so the erased intermediate state gets rendered and the text visibly
// blinks out and back on every reflow. To keep the repaint atomic, output
// that follows a resize is held briefly and written as one batch wrapped in
// DEC private mode 2026 (synchronized output): xterm parses the whole batch
// without painting — even when the parser yields mid-batch — and paints the
// settled screen in a single frame. The markers travel inside the data
// stream, so the freeze can never catch the renderer's own resize (which
// clears the canvas outside the mode) or outlive the batch; xterm would
// force-release a stuck freeze after 1s regardless.
const RESIZE_REPAINT_FIRST_MS = 150; // repaint never started: give up waiting
const RESIZE_REPAINT_QUIET_MS = 35; // no further chunk: repaint is complete
const RESIZE_REPAINT_MAX_MS = 300; // stream never goes quiet: cap the delay
...
class TerminalElement extends HTMLElement {
  ...
  // PTY output held back while ConPTY repaints after a resize; `null` when
  // output flows straight through. See `#beginResizeRepaintHold`.
  #resizeRepaintChunks = null;
  #resizeRepaintTimer;
  #resizeRepaintDeadline = 0;
  ...
  async initialize(model) {
    ...
    try {
      ...
      // Track the pane divider frame by frame. The terminal itself refits
      // live — otherwise its canvas keeps the old geometry for the whole
      // drag, clipping away the bottom rows (the prompt) as the pane shrinks
      // and snapping everything back on the eventual refit, which reads as
      // the text blinking out and in on every reflow. Only the PTY is told
      // about the new size on a debounce: a full-screen application repaints
      // on every resize it hears about, and it only needs the final one.
      let debouncedResizePty = debounce(() => this.resizePtyToTerminal());
  ...
  async restartPtyProcess() {
    ...
    // A repaint hold still open for the outgoing shell must not swallow its
    // final output or delay the replacement's first.
    this.#flushResizeRepaintHold();
  ...
  // Fit xterm to the container. Cheap for a height change (rows are added or
  // removed); a width change rewraps the buffer, which xterm handles
  // incrementally fast enough to run per frame.
  fitTerminal() {
  ...
  resizePtyToTerminal() {
    ...
    this.#beginResizeRepaintHold();
    this.pty.resize(geometry.cols, geometry.rows);
    this.#ptyMeta.cols = geometry.cols;
    this.#ptyMeta.rows = geometry.rows;
  }

  // The single entry point for PTY output. While a resize repaint is being
  // held, chunks accumulate instead of being written; each arrival pushes the
  // flush out a little, so the batch closes on the first quiet gap.
  #writeToTerminal(data) {
    if (this.#resizeRepaintChunks) {
      this.#resizeRepaintChunks.push(data);
      this.#scheduleResizeRepaintFlush(RESIZE_REPAINT_QUIET_MS);
      return;
    }
    this.terminal?.write(data);
  }

  #beginResizeRepaintHold() {
    this.#resizeRepaintChunks ??= [];
    this.#resizeRepaintDeadline = Date.now() + RESIZE_REPAINT_MAX_MS;
    this.#scheduleResizeRepaintFlush(RESIZE_REPAINT_FIRST_MS);
  }

  #scheduleResizeRepaintFlush(waitMs) {
    clearTimeout(this.#resizeRepaintTimer);
    let remaining = this.#resizeRepaintDeadline - Date.now();
  ...
  #flushResizeRepaintHold() {
    clearTimeout(this.#resizeRepaintTimer);
    this.#resizeRepaintTimer = undefined;
    let chunks = this.#resizeRepaintChunks;
    this.#resizeRepaintChunks = null;
    if (chunks?.length) {
