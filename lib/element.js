...
// terminal.
function clampLineHeight(lineHeight, fontSize, domWindow = globalThis.window) {
  let lineHeightInPx = fontSize * lineHeight;
  let roundedScaledLineHeightInPx = Math.round(lineHeightInPx * domWindow.devicePixelRatio);
  return roundedScaledLineHeightInPx / (fontSize * domWindow.devicePixelRatio);
}
...
function redispatchKeyboardEvent(originalEvent, targetElement) {
  const KeyboardEvent = targetElement.ownerDocument.defaultView.KeyboardEvent;
...
const TAG_NAME = "terminal-view";

function createTerminalElementClass({ HTMLElement }) {
  return class TerminalElement extends HTMLElement {
    ...
    static create(document = globalThis.document) {
    ...
    async initialize(model) {
      ...
      try {
        ...
        this.#startSurfaceObservers();
        this.subscriptions.add(new Disposable(() => this.#stopSurfaceObservers()));
    ...
    #startSurfaceObservers() {
      this.#stopSurfaceObservers();
      const domWindow = this.domWindow;
      if (!domWindow || !this.div?.main) return;

      this.#mainResizeObserver = new domWindow.ResizeObserver((entries) => {
        let last = entries[entries.length - 1];
        this.#mainContentRect = last.contentRect;
        this.#scheduleLiveFit();
        clearTimeout(this.#ptyResizeTimer);
        this.#ptyResizeTimer = domWindow.setTimeout(
      ...
      this.#terminalIntersectionObserver = new domWindow.IntersectionObserver(
        async (entries) => {
          let last = entries[entries.length - 1];
    ...
    async beginWindowSurfaceTransition(context) {
      ...
      if (this.#surfaceTransition) {
        throw new Error("A terminal window-surface transition is already in progress");
      }
      const state = {
        id: context.id,
        wasFocused: this.contains(this.ownerDocument.activeElement),
      };
      ...
      this.#ligaturesAddon?.dispose();
      this.#ligaturesAddon = undefined;
      this.#imageAddon?._renderer?.removeLayerFromDom?.();
    ...
    async #finishWindowSurfaceTransition(state) {
      ...
      const domWindow = this.domWindow;
      if (!domWindow) throw new Error("The terminal is not attached to a live window");

      if (this.terminal) {
        const core = this.terminal._core;
        if (core?._coreBrowserService) core._coreBrowserService.window = domWindow;
        core?._renderService?.handleDevicePixelRatioChange?.();
        this.#loadWebglAddon();
        this.#imageAddon?._renderer?.insertLayerToDom?.();
        this.#loadLigaturesAddon();
        this.themeSignature = null;
        this.updateTheme();
        const rect = this.div.main.getBoundingClientRect();
        this.#mainContentRect = { width: rect.width, height: rect.height };
        this.refitTerminal();
      ...
      if (state.wasFocused) this.terminal?.focus();
    }
    ...
    #keyboardEventMatchesKeybinding(event) {
      ...
      while (node && node.matches) {
        ...
        if (node.parentNode === this.ownerDocument) break;
    ...
    getXtermOptions() {
      ...
      if (xtermOptions.fontSize) {
        let adjustedLineHeight = clampLineHeight(
          originalLineHeight,
          xtermOptions.fontSize,
          this.domWindow,
        );
        xtermOptions.lineHeight = adjustedLineHeight;
      }
      xtermOptions.theme = getTheme(this.ownerDocument);
    ...
    setMainBackgroundColor(theme = getTheme(this.ownerDocument)) {
    ...
    // The single activation path for every clickable link: OSC 8 hyperlinks
    // (via the `linkHandler` xterm option) and plain-text URLs (via the
    // web-links addon) both land here, so the modifier guard applies uniformly.
    async activateLink(event, uri) {
      ...
      if (!uri.startsWith("file:")) {
        // A URL, most likely. Hand it to the system for opening in the user's
        // default browser.
      ...
      if (fs.statSync(linkPath).isDirectory()) {
        // The system opens a directory URI in its file explorer.
        lumine.shell.openExternal(uri);
      } else {
        await lumine.workspace.open(linkPath);
      }
    }
    ...
    async #createTerminal() {
      ...
      this.findPalette = new FindPalette(this.#searchAddon, { document: this.ownerDocument });
    ...
    updateTheme() {
      ...
      let theme = getTheme(this.ownerDocument);
...
function registerTerminalElement() {
  ...
  lumine.elements.define(TAG_NAME, createTerminalElementClass);
  elementRegistered = true;
}

registerTerminalElement();
const TerminalElement = globalThis.customElements.get(TAG_NAME);

module.exports = { TerminalElement, createTerminalElementClass, registerTerminalElement };
